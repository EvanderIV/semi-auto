...
io.on('connection', client => {
    ...
    client.on('join-room', (data) => {
        ...
        if (room.players.size >= 8) { // Increased max players for Semi-Auto
    ...
    client.on('playerTap', () => {
        ...
        // Broadcast the tap event to all players in the room
        io.to(roomCode).emit('tapEvent', {
            playerId: client.id,
    ...
    client.on('updateGameState', (data) => {
        if (!roomCode) return;
        ...
        io.to(roomCode).emit('gameStateUpdate', data);
    });
